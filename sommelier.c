...
static const struct xdg_wm_base_listener xwl_internal_xdg_shell_listener = {
...
static void xwl_set_input_focus(struct xwl *xwl, struct xwl_window *window) {
  if (window) {
    ...
    static void
    xwl_restack_windows(struct xwl *xwl, uint32_t focus_resource_id) {
...
static void xwl_internal_xdg_surface_configure(
  ...
  if (!window->pending_config.serial) {
    ...
    DEBUG_PRINT;
    if (host_resource) host_surface = wl_resource_get_user_data(host_resource);
...
static void xwl_internal_xdg_toplevel_configure(
  ...
  wl_array_for_each(state, states) {
    ...
    if (*state == XDG_TOPLEVEL_STATE_TILED_BOTTOM) {  // FIXME: is this right???
      window->allow_resize = 0;
      window->next_config.states[i++] =
    ...
    if (*state == XDG_TOPLEVEL_STATE_ACTIVATED)
      activated = 1;
    if (*state == XDG_TOPLEVEL_STATE_RESIZING) window->allow_resize = 0;
  }
...
static const struct xdg_toplevel_listener xwl_internal_xdg_toplevel_listener =
    {xwl_internal_xdg_toplevel_configure, xwl_internal_xdg_toplevel_close};
...
static void xwl_window_update(struct xwl_window *window) {
  ...
  if (window->managed || !parent) {
    ...
    positioner = xdg_wm_base_create_positioner(xwl->xdg_shell->internal);
    assert(positioner);
    xdg_positioner_set_anchor(
        positioner, XDG_POSITIONER_ANCHOR_TOP | XDG_POSITIONER_ANCHOR_LEFT);
...
static const struct xdg_wm_base_interface xwl_xdg_shell_implementation = {
    xwl_xdg_shell_destroy, xwl_xdg_shell_create_positioner,
    xwl_xdg_shell_get_xdg_surface, xwl_xdg_shell_pong};

static void xwl_xdg_shell_ping(void *data, struct xdg_wm_base *xdg_shell,
                               uint32_t serial) {
  // struct xwl_host_xdg_shell *host = xdg_wm_base_get_user_data(xdg_shell);

  xdg_wm_base_pong(xdg_shell, serial);
}

static const struct xdg_wm_base_listener xwl_xdg_shell_listener = {
    xwl_xdg_shell_ping};
...
static void xwl_bind_host_xdg_shell(struct wl_client *client, void *data,
  ...
  host->proxy =
      wl_registry_bind(wl_display_get_registry(xdg_shell->xwl->display),
                       xdg_shell->id, &xdg_wm_base_interface, 1);
  xdg_wm_base_set_user_data(host->proxy, host);
  xdg_wm_base_add_listener(host->proxy, &xwl_xdg_shell_listener, host);
}
  ...
  if (strcmp(interface, "wl_compositor") == 0) {
    ...
    if (xwl->xwayland) {
      ...
      xdg_shell->host_global = xwl_global_create(
          xwl, &xdg_wm_base_interface, 1, xdg_shell, xwl_bind_host_xdg_shell);
    }
    assert(!xwl->xdg_shell);
...
static void xwl_registry_remover(void *data, struct wl_registry *registry,
  ...
  if (xwl->xdg_shell && xwl->xdg_shell->id == id) {
    ...
    if (xwl->xdg_shell->internal) xdg_wm_base_destroy(xwl->xdg_shell->internal);
    free(xwl->xdg_shell);
    xwl->xdg_shell = NULL;
    return;
  }
